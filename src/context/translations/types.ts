export type TranslationContextType = {
  lang: Lang;
  setLang: (lang: Lang) => void;
  translations: Translations;
};

export type Lang = "en" | "es" | "cn" | "ru";

export type ProductKey =
  | "beef"
  | "lamb"
  | "horse"
  | "dairy"
  | "poultry"
  | "grains"
  | "rice"
  | "animalFeed";

export type ProductDetail = {
  summary: string;
  highlights: readonly string[];
  logistics: string;
};

export interface Translations {
  about: string;
  products: string;
  globalReach: string;
  contact: string;
  getInTouch: string;
  heroTitle: string;
  heroSubtitle: string;
  exploreProducts: string;
  contactUs: string;
  aboutTitle: string;
  aboutText1: string;
  aboutText2: string;
  qualityGuaranteed: string;
  qualityText: string;
  competitivePricing: string;
  pricingText: string;
  productsTitle: string;
  productsSubtitle: string;
  beef: string;
  beefDesc: string;
  lamb: string;
  lambDesc: string;
  horse: string;
  horseDesc: string;
  dairy: string;
  dairyDesc: string;
  poultry: string;
  poultryDesc: string;
  rice: string;
  riceDesc: string;
  grains: string;
  grainsDesc: string;
  animalFeed: string;
  animalFeedDesc: string;
  viewAll: string;
  learnMore: string;
  productHighlightsLabel: string;
  productLogisticsLabel: string;
  close: string;
  productDetails: Record<ProductKey, ProductDetail>;
  globalTitle: string;
  globalSubtitle: string;
  supplierCountries: string;
  supplierText: string;
  clientMarkets: string;
  clientText: string;
  andMore: string;
  valueTitle: string;
  valueSubtitle: string;
  industryExpertise: string;
  industryExpertiseText: string;
  trustedNetwork: string;
  trustedNetworkText: string;
  reliabilityCommitment: string;
  reliabilityCommitmentText: string;
  executionExcellence: string;
  executionExcellenceText: string;
  partnershipApproach: string;
  partnershipApproachText: string;
  contactTitle: string;
  contactText: string;
  contactFormTitle: string;
  fullName: string;
  company: string;
  email: string;
  productsInterest: string;
  selectProduct: string;
  message: string;
  messagePlaceholder: string;
  sendMessage: string;
  yourName: string;
  yourCompany: string;
  yourEmail: string;
  footerText: string;
  productsFooter: string;
  companyFooter: string;
  aboutUs: string;
  globalReachFooter: string;
  certifications: string;
  sustainability: string;
  legal: string;
  terms: string;
  privacy: string;
  cookies: string;
  rights: string;
  unitedStates: string;
  europeanUnion: string;
  unitedKingdom: string;
  philippines: string;
}

export type TranslationsKeys = keyof Translations;
